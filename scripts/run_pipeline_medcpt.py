"""
Run the MedCPT-only dense RAG pipeline on BioASQ data

Usage:
  python scripts/run_pipeline_medcpt.py --round 1 --email user@example.com --config configs/pipeline_config.yaml --data-dir data --output results --max-questions 10
"""
...
def run_pipeline(questions: List[Dict[str, Any]], documents: List[Dict[str, Any]], config_path: str) -> List[Dict[str, Any]]:
    logger.info("Initializing MedCPT-only RAG pipeline...")
    import yaml
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)
    pipeline = MedicalRAGPipelineMedCPT(config)
    logger.info(f"Indexing {len(documents)} documents into FAISS (MedCPT embeddings)...")
...
def evaluate_results(predictions: List[Dict[str, Any]], golden_data: Dict[str, Any], use_llm_judge: bool = False) -> Dict[str, float]:
    ...
    logger.info("Evaluating predictions (MedCPT retriever)...")
...
def save_results(predictions: List[Dict[str, Any]], metrics: Dict[str, float], output_dir: str, round_num: int):
    ...
    pred_file = os.path.join(base_dir, "predictions_medcpt.json")
    metrics_file = os.path.join(base_dir, "metrics_medcpt.json")
    with open(pred_file, "w") as f:
        json.dump(predictions, f, indent=2)
    with open(metrics_file, "w") as f:
        json.dump(metrics, f, indent=2)
    logger.info(f"Saved MedCPT predictions to {pred_file}")
    logger.info(f"Saved MedCPT metrics to {metrics_file}")


def main():
    parser = argparse.ArgumentParser(description="Run MedCPT-dense RAG pipeline on BioASQ data")
